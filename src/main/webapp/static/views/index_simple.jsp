...
<script>
            ...
            success: function(data) {
                ...
                if(data && data.list){
                    ...
                    $.each(data.list, function(index, product) {
                        ...
                        var quantityInput = $('<input>').attr('type', 'number').attr('value', '1')
                            .attr('min', '1').addClass('form-control');
                        // 为添加到购物车的按钮添加点击事件
                        addToCartButton.click(function() {
                            // 在这里执行您希望当按钮被点击时发生的操作
                            //alert('产品:' + product.pName + ' 已添加到购物车!');
                            addShoppingCart('/Restaurant/add_shopping_cart',product.pName,product.pName,
                                '${message.account}',product.pShopper,quantityInput.val(),product.pPrice);
                        });
                        productInfo.append(nameElement, priceElement, descriptionElement,quantityInput,addToCartButton);
    ...
    function addShoppingCart(url,title,content,creator,shopper,number,tradingPrice){
        $.ajax({
            url:url,
            type: 'GET',
            data: {
                title:title,
                content: content+number+"份",
                creator: creator,
                shopper: shopper,
                number:number,
                tradingPrice: tradingPrice*number
            },
            //dataType: 'json', //服务器返回JSON格式
            success: function(data) {
                alert('产品:' + product.pName + '已添加到购物车!');
